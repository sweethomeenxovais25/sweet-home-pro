    ...
    with st.form("form_upload_cloudinary", clear_on_submit=True):
        ...
        if st.form_submit_button("Salvar no Cofre 🔒"):
            ...
            if not erro:
                ...
                # 💡 Extrai a extensão original (.pdf, .jpg) e cola no nome limpo
                import os
                extensao = os.path.splitext(arquivo_subido.name)[1].lower()
                nome_limpo_com_ext = f"{nome_limpo}{extensao}"

                with st.spinner(f"Subindo para o servidor seguro... ⏳"):
                    # 1. Tenta fazer o upload para o Cloudinary (agora com a extensão correta)
                    f_id, f_link = upload_para_cloudinary(arquivo_subido.getvalue(), nome_limpo_com_ext, cat_escolhida)
...
# ==========================================================
    ...
    if not df_vendas_hist.empty:
        ...
        with st.expander(f"📝 Gerar e Comprovar Declaração Anual (DASN-SIMEI)", expanded=False):
            ...
            # --- ABA DE HISTÓRICO DAS DECLARAÇÕES ---
            with tab_dasn_hist:
                ...
                if not df_cont.empty:
                    ...
                    if not df_dasn.empty:
                        # Extrai os anos da competência para o filtro (Ex: "Ano-Calendário 2025" -> "2025")
                        ...
                        if filtro_dasn != "Todos os Anos":
                            df_dasn = df_dasn[df_dasn['COMPETENCIA'].str.contains(filtro_dasn, na=False)]
                        
                        # Organiza do mais recente para o mais antigo
                        df_dasn = df_dasn.iloc[::-1]
                        
                        st.dataframe(
                            df_dasn[['DATA_PAGAMENTO', 'COMPETENCIA', 'STATUS', 'LINK_COMPROVANTE']],
                            column_config={
                                "DATA_PAGAMENTO": "Data de Envio/Upload",
                                "COMPETENCIA": "Ano Base da Declaração",
                                "STATUS": "Status",
                                "LINK_COMPROVANTE": st.column_config.LinkColumn("Visualizar Recibo Oficial (PDF)")
                            },
                            use_container_width=True, hide_index=True
                        )
                    else:
                        st.info("Nenhuma Declaração Anual (DASN) foi guardada no cofre até ao momento.")
                else:
                    st.info("O banco de dados contábil está vazio.")
    else:
            ...
            if st.form_submit_button("Confirmar Pagamento e Integrar 🔒", type="primary"):
                if comp_arquivo:
                    with st.spinner("Calculando impacto financeiro e arquivando..."):
                        ...
                        # 💡 Adiciona a extensão ao nome da guia
                        import os
                        ext_das = os.path.splitext(comp_arquivo.name)[1].lower()
                        nome_doc = f"DAS_{comp_mes}_{ano_selecionado}_Atraso{dias_atraso}d{ext_das}"
